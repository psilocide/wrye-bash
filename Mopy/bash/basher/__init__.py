    ...
    def _activeModsFirst(self, items):
        if self.selectedFirst:
            ...
            items.sort(key=lambda x: self.data[x].name not in set(
                active) | bosh.modInfos.imported | bosh.modInfos.merged)
    extra_sortings = [_ModsSortMixin._sortEsmsFirst, _activeModsFirst]
...
class INIList(List):
    ...
    _shellUI = True
    sort_keys = {'File': None,
                 'Installer': lambda _self, a: bosh.iniInfos.table.getItem(
                     a, 'installer', u''),
                }
    ...
    def PopulateItem(self,itemDex,mode=0,selected=set()):
        fileName, itemDex = super(INIList, self).PopulateItem(itemDex, mode, selected)
...
class ModList(_ModsSortMixin, List):
    ...
    sort_keys = {
        'File': None,
        'Author': lambda self_, a: self_.data[a].header.author.lower(),
        'Rating': lambda self_, a: bosh.modInfos.table.getItem(
                     a, 'rating', u''),
        'Group': lambda self_, a: bosh.modInfos.table.getItem(a, 'group', u''),
        'Installer': lambda self_, a: bosh.modInfos.table.getItem(
                     a, 'installer', u''),
        # FIXME(ut): quadratic + accessing modInfos.plugins which is private
        'Load Order': lambda self_, a: a in bosh.modInfos.plugins.LoadOrder and
                                      bosh.modInfos.plugins.LoadOrder.index(a),
        'Modified': lambda self_, a: self_.data[a].getPath().mtime,
        'Size': lambda self_, a: self_.data[a].size,
        'Status': lambda self_, a: self_.data[a].getStatus(),
        'Mod Status': lambda self_, a: self_.data[a].txt_status(),
        'CRC': lambda self_, a: self_.data[a].cachedCrc(),
    }
    ...
    def PopulateItem(self,itemDex,mode=0,selected=set()):
        fileName, itemDex = super(ModList, self).PopulateItem(itemDex, mode, selected)
        fileInfo = self.data[fileName]
        listCtrl = self._gList
        #--Image
        status = fileInfo.getStatus()
...
class SaveList(List):
    ...
    sort_keys = {'File'    : None, # just sort by name
                 'Modified': lambda self_, a: self_.data[a].mtime,
                 'Size'    : lambda self_, a: self_.data[a].size,
                 'Status'  : lambda self_, a: self_.data[a].getStatus(),
                 'Player'  : lambda self_, a: self_.data[a].header.pcName,
    ...
    def PopulateItem(self,itemDex,mode=0,selected=set()):
        fileName, itemDex = super(SaveList, self).PopulateItem(itemDex, mode, selected)
        fileInfo = self.data[fileName]
        #--Image
        status = fileInfo.getStatus()
...
class ScreensList(List):
    ...
    editLabels = True
    sort_keys = {'File'    : None,
                 'Modified': lambda self_, a: self_.data[a][1],
                }
